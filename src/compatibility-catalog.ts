...
export interface CatalogAcceptance { status: "accepted" | "rejected" | "invalid"; reason: string; catalog?: CompatibilityCatalog; }
...
export function acceptCompatibilityCatalog(input: unknown, roots: Readonly<Record<string, string>> = COMPATIBILITY_ROOTS, acceptedVersion = 0, now = new Date(), revokedKeys: ReadonlySet<string> = REVOKED_COMPATIBILITY_KEYS): CatalogAcceptance {
  ...
  if (!verify(null, Buffer.from(canonicalCatalogJson(catalog)), createPublicKey(root), Buffer.from(signature, "base64"))) return { status: "invalid", reason: "Catalog signature did not verify" };
  ...
  if (catalog.catalogVersion <= acceptedVersion) return { status: "rejected", reason: "Catalog version is not newer than the accepted version" };
  return { status: "accepted", reason: "Signature, schema, timestamps, and version accepted", catalog };
}
