...
export class DeliveryService {
  ...
  async execute(input: DeliveryExecutionInput): Promise<DeliveryRepositoryRecord> {
    ...
    try {
      ...
      const truthful: DeliveryRepositoryRecord = { ...delivery, status: "tagged", remoteUrl: remote.webUrl, approvalId: input.approval.id, error: null, releaseTag: input.tag!, updatedAt: delivery.updatedAt }; try { let eventAttempted = false; const persistTagged = () => {
      const updated = this.ledger.updateDeliveryRepository(input.runId, input.repositoryId, {
      ...
      return updated;
      }; try { return persistTagged(); } catch (error) { if (eventAttempted) { deliveryDiagnostic(`release tag '${input.tag}' was pushed but its delivery.tagged event could not be recorded (${error instanceof Error ? error.message : String(error)})`); return truthful; } deliveryDiagnostic(`release tag '${input.tag}' was pushed but tagged status persistence failed; retrying once (${error instanceof Error ? error.message : String(error)})`); try { return persistTagged(); } catch (retryError) { deliveryDiagnostic(eventAttempted ? `release tag '${input.tag}' was pushed but its delivery.tagged event could not be recorded (${retryError instanceof Error ? retryError.message : String(retryError)})` : `release tag '${input.tag}' is public but tagged status reconciliation still requires owner repair (${retryError instanceof Error ? retryError.message : String(retryError)})`); return truthful; } } } catch { deliveryDiagnostic(`release tag '${input.tag}' is public but local reconciliation encountered an unexpected failure`); return truthful; }
      });
    } catch (error) {
