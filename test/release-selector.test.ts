import assert from "node:assert/strict"; import { execFile } from "node:child_process"; import { link, mkdir, mkdtemp, readFile, rename, rm, writeFile } from "node:fs/promises";
...
test("nested authority automatically selects CivicRecords-style backend with excluded provenance", async () => {
  ...
  const root = await mkdtemp(path.join(os.tmpdir(), "dh-selector-lock-")), filename = path.join(root, "ledger.db"), hardlink = path.join(root, "hardlink.db"); const ledger = new Ledger(filename); let stale = "", shadow = "", diagnostics = 0, frozen = false, originalConsole = console.error;
  try { register(ledger, root); await link(filename, hardlink); let hardError: unknown; try { await ledger.withReleaseUnitLock("repo:hard", async () => undefined); } catch (caught) { hardError = caught; } await rm(hardlink);
    ...
    let value: unknown, cleanupError: unknown, nonceError: unknown; try { value = await ledger.withReleaseUnitLock("repo:test", async (lock) => { frozen = Object.isFrozen(lock); stale = lock.key; await writeFile(stale, "tampered"); try { ledger.updateReleaseUnitSelection("repo:test", "b", 0, lock); } catch (caught) { nonceError = caught; } return "completed"; }); } catch (caught) { cleanupError = caught; } const nonceReplacement = await readFile(stale, "utf8"); await rm(stale); stale = "";
    let inodeError: unknown; await ledger.withReleaseUnitLock("repo:test", async (lock) => { stale = lock.key; const nonce = await readFile(stale, "utf8"), replacement = `${stale}.replacement`; await writeFile(replacement, nonce); await rm(stale); await rename(replacement, stale); try { ledger.updateReleaseUnitSelection("repo:test", "b", 0, lock); } catch (caught) { inodeError = caught; } }); const inodeReplacement = await readFile(stale, "utf8"); await rm(stale); stale = "";
    ...
    const errorReplacement = await readFile(stale, "utf8"); await rm(stale); stale = ""; let escaped: any; await ledger.withReleaseUnitLock("repo:test", async (lock) => { escaped = lock; stale = lock.key; shadow = `${stale}.shadow`; await link(stale, shadow); }); await link(shadow, stale); let revokedError: unknown; try { ledger.updateReleaseUnitSelection("repo:test", "b", 0, escaped); } catch (caught) { revokedError = caught; } await rm(stale); stale = ""; await rm(shadow); shadow = ""; console.error = originalConsole;
    assert.deepEqual({ hardlink: /hard.?link/i.test(String(hardError)), forged: /already in progress/i.test(String(forgedError)), frozen, value, cleanupError: cleanupError ?? null, nonce: /already in progress/i.test(String(nonceError)), nonceReplacement, inode: /already in progress/i.test(String(inodeError)), inodeReplacement: inodeReplacement.length > 0, diagnostics, original: /callback sentinel/.test(String(originalError)), errorReplacement, revoked: /already in progress/i.test(String(revokedError)) }, { hardlink: true, forged: true, frozen: true, value: "completed", cleanupError: null, nonce: true, nonceReplacement: "tampered", inode: true, inodeReplacement: true, diagnostics: 3, original: true, errorReplacement: "replacement-error", revoked: true });
  } finally { console.error = originalConsole; if (stale) await rm(stale, { recursive: true, force: true }); if (shadow) await rm(shadow, { force: true }); ledger.close(); await rm(root, { recursive: true, force: true }); }
