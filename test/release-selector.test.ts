...
function register(ledger: Ledger, localPath: string, intelligence: Record<string, unknown> = {}) { ledger.upsertProduct({ id: "product:test", name: "Test", organizationUrl: "https://example.invalid/test", description: "fixture", repositories: [] });
...
test("nested authority automatically selects CivicRecords-style backend with excluded provenance", async () => {
  ...
  try { const authority: any = await new DeliveryService(ledger).versionAuthorityAtCommit(fixture.root, fixture.commit);
    assert.deepEqual({ state: authority.state, version: authority.version, source: authority.source, cwd: authority.cwd, reason: authority.reason,
      units: authority.units.map((unit: any) => [unit.cwd, unit.state]) }, { state: "declared", version: "1.7.3",
...
test("typed selector CAS is durable, race-safe, stale-safe, and reserved across metadata refresh", async () => {
  ...
  let ledger = new Ledger(filename); try {
    ...
    assert.deepEqual(ledger.getRepository(original.id)?.intelligence.releaseUnitSelection, original.intelligence.releaseUnitSelection, "metadata upsert preserves the reserved selector");
    ...
    assert.deepEqual(ledger.getRepository(original.id)?.intelligence.releaseUnitSelection, original.intelligence.releaseUnitSelection, "product metadata upsert also preserves the reserved selector");
    ...
    const refresher = race[0].status === "fulfilled" ? contender : ledger; register(refresher, root, { ...original.intelligence, staleRefresh: true });
...
test("malformed selection fails closed", async () => {
  ...
  const ledger = new Ledger(path.join(fixture.root, "ledger.db"));
  try {
    register(ledger, fixture.root, { releaseUnitSelection: { version: 99, cwd: "a" } }); const service = new DeliveryService(ledger);
...
test("selection decoding distinguishes absence, valid v1, malformed, and unknown data", () => {
  ...
  assert.equal(decodeReleaseUnitSelection({ releaseUnitSelection: { ...active, selectedAt: "2026-07-25" } }).kind, "malformed");
  assert.equal(decodeReleaseUnitSelection({ releaseUnitSelection: active }).kind, "valid");
});
