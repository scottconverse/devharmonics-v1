...
The tag field is pre-filled from the exact immutable commit being delivered. DevHarmonics uses a usable root `package.json` version unless that manifest declares boolean `"private": true`; a private npm version is treated as internal tooling metadata and resolution falls through to the root PEP 621 `[project].version`. If neither root manifest makes an authoritative release claim, the card reports **no authoritative release version** and leaves the tag for you to enter or skip.
