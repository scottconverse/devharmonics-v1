...
The tag field is pre-filled from the exact immutable commit being delivered. DevHarmonics uses a valid public root `package.json` version; a private package or valid package without a version falls through to standards-parsed PEP 621 `project.version`. The card reports four states honestly: a **declared** version is pre-filled and mismatch-checked; **no authoritative release version** means both root manifests were valid or absent but declared no usable static version, so you may enter or skip a tag; an **invalid** manifest must be fixed; and **release authority unavailable** means the immutable Git data could not be read safely and should be retried. Invalid and unavailable states disable and refuse tagging, including the tag step inside **Do everything at once**; mismatch confirmation cannot bypass them.
