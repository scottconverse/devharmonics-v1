...
| PS-CAP-036 | Reconcile signed-in/runtime reports, official catalogs, signed compatibility catalog, and local empirical results. | SHOULD | `docs/PRODUCT_SPEC.md:447-454` | PARTIAL — signed, versioned compatibility metadata is verified and persisted; empirical reconciliation remains DH-250 scope | `src/compatibility-catalog.ts`; `src/catalog.ts`; signed-catalog core test |
...
| PS-MS-C01 | Automated catalog refresh/signed compatibility metadata. | MILESTONE | `docs/PRODUCT_SPEC.md:1316` | PARTIAL — DH-220 now verifies bundled Ed25519 metadata, persists trust receipts, marks invalid evidence stale, and revokes active qualifications; live catalog delivery and broader provider integration remain | signed-catalog/fingerprint core tests; TypeScript build |
